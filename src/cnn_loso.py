"""
cnn_loso.py — 1D CNN with Leave-One-Subject-Out Cross-Validation
==================================================================
Trains a lightweight 1D CNN on raw inertial signals (128, 9)
...
MAX_EPOCHS = 20
PATIENCE = 3
RF_BASELINE = 0.8988  # From Phase 2A


def run_cnn_loso():
    """Run full 30-fold LOSO evaluation with 1D CNN."""
    ...
    print("\n" + "=" * 65)
    print("  1D CNN — 30-FOLD LOSO EVALUATION")
    print(f"  Model: Conv1D(64)→Conv1D(128)→GAP→Dense(64)→Dense(6)")
    print(f"  Params: ~36K | Epochs: {MAX_EPOCHS} max | EarlyStopping: patience={PATIENCE}")
    print("=" * 65)
    ...
    print()
    print(f"  ── Comparison to RF Baseline ──")
    print(f"  RF  baseline:             {RF_BASELINE:.4f}")
    print(f"  CNN result:               {mean_acc:.4f}")
    delta = mean_acc - RF_BASELINE
    print(f"  Delta:                    {delta:+.4f} ({'↑ IMPROVEMENT' if delta > 0 else '↓ REGRESSION'})")
    print("=" * 65)

    # ── Save to file ──
    save_path = os.path.join(RESULTS_DIR, "cnn_loso_results.txt")
    with open(save_path, "w", encoding="utf-8") as f:
        f.write("1D CNN — LOSO Results\n")
        f.write("=" * 50 + "\n")
        f.write(f"Architecture: Conv1D(64)->Conv1D(128)->GAP->Dense(64)->Dense(6)\n")
